...
public class UserRoomsDAOImplem implements UserRoomsDAO{
    ...
    public void insertUserRoom(int userId, int roomId, int mapId, Integer north, Integer south, Integer east, Integer west, Integer coins, Integer doorKeyId) {
        jdbcTemplate.update("insert into userRooms (userId, roomId, mapId, north, south, east, west, coins, doorkeyId) values (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    public void updateTotalCoins(int userId, int roomId, int roomTotalCoins) {
        jdbcTemplate.update("UPDATE userRooms SET coins = ? WHERE roomId = ? AND userId = ?", roomTotalCoins, roomId, userId);
    }

    @Override
    public void updateTotalKeys(int userId, int roomId, int roomTotalKeys) {
        jdbcTemplate.update("UPDATE userRooms SET doorKeyId = ? WHERE roomId = ? AND userId = ?", 0, roomId, userId);
    }
